...
def admin_login():
    ...
    # MySQL may return bytes or string
...
def user_pay():
    ...
    selected_ids = session.get('selected_cart', [])
    ...
    # Calculate selected products total only
    total_amount = sum(
...
def verify_payment():
    ...
    try:
        ...
        for pid_str, item in selected_cart.items():
...
#########################Checkout selected###################################
@app.route('/user/checkout-selected', methods=['POST'])
def checkout_selected():
    ...
    session['selected_cart'] = selected_items
